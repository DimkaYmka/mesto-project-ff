...
const createNewCard = (cardData, openImagePopup, userId) => {
  const { name, link, cardId, likes } = cardData; // Деструктурируем объект cardData
  ...
  const elementsLike = newCard.querySelector('.card__like-button');
  elementsLike.addEventListener('click', () => {
    likeFunction(cardId, elementsLike, userId);
  });
...
const likeFunction = (cardId, elementsLike, userId) => {
  // Используйте cardId вместо elementsLike.dataset.cardId
  const isLiked = elementsLike.classList.contains('card__like-button_is-active');
  const apiMethod = isLiked ? deleteLike : addLike;
...
const deleteFunction = (cardToDelete, cardId, userId) => {
  deleteCard(cardId)
    .then(data => {
      console.log('Card deleted successfully:', data);
      cardToDelete.remove();
...
const renderCard = (cardData, openImagePopup, userId) => {
  const { name, link, _id, likes } = cardData; // Используйте _id вместо cardId
  const newCard = createNewCard({ name, link, cardId: _id, likes }, openImagePopup, userId);
  cardsConteiner.prepend(newCard);
};
